...
- [x] Parse exact reset time from claude plan-limit errors for precise cooldowns. — Added `_parse_reset_to_epoch` + helpers (`__time_to_24h`, `__date_parse`, `__date_add_days`, `__parse_time_only`, `__parse_day_time`) to `bin/backlog-agent`. Extracts `resets 3:45pm` or `resets Mon 12:00am` strings from claude's captured output and converts them to a Unix epoch. `start_cooldown()` now accepts an optional epoch argument — if parsing succeeds the cooldown expires at the exact reset moment instead of guessing 1 hour. `agent-cooldown.json` gains a `parsed_from_reset` boolean for observability. Falls back to `COOLDOWN_SECONDS=3600` if parsing fails.
- [x] Lazy daemon — escalate idle sleep, sleep precisely on cooldown. — Replaced the fixed 300s→1800s heartbeat with an escalating idle back-off: 30s→60s→120s→300s→600s→1800s. Resets to fast-start as soon as work appears. fswatch still fires instantly on user edits to `Backlog.md`. Cooldown sleep uses the exact parsed reset time instead of polling every 30min. New: `ESCALATING_IDLE_SLEEP` constant, `_pick_idle_sleep` helper, `TICK_OUTCOME` (work|idle|cooldown) variable set by `tick_once` at each exit, `COOLDOWN_WAIT_SECONDS` for exact wake. `do_run` / `do_watch` dispatch sleep based on outcome state. Status hooks drop from ~120/hr to ~12/hr per project when idle.
- [x] Dashboard: label cooldowns as precise vs estimated using `parsed_from_reset`. — `functions/api/fleet.js` now exposes `cooldown.parsed` (boolean). FLEET tab tooltip shows "(precise)" or "(estimated)". TOKENS tab cooldown section shows "precise N min" vs "~ N min" with the until-time.
- [x] Dashboard: manual refresh button. — ↻ button in the top-right corner with a CSS spin animation on click. Calls `forceRefresh()` which triggers `refresh()` and removes the spinning class on completion. Keyboard shortcut: `r`.
- [x] CLI `backlog-agents`: show cooldown remaining time + reason with precision tag. — Health alerts now read `parsed_from_reset` from `agent-cooldown.json`. Output format: `project-name (precise) cooldown (reason) — 47m left (until 15:45)` or `(~estimated)` for the 1h fallback.
- [x] Dashboard: keyboard shortcuts + help overlay. — Press `?` to toggle a help overlay showing all shortcuts. Press `1`-`7` to switch tabs (PROJECTS through TOKENS). `p` pauses/resumes auto-refresh. `r` refreshes. `/` searches pending. `j`/`k` navigate pending items. Esc closes help overlay.
- [x] Dashboard: auto-refresh toggle button. — ⟳ button toggles the 5-min polling interval. Shows red border when paused. Subtitle line updates from "every 5 min" to "paused". Keyboard shortcut: `p`.
